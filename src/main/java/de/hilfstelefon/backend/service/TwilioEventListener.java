...
public class TwilioEventListener {
    ...
    @ConsumeEvent(value = TwilioGatherTranscriptionCompleted.EVENTNAME_REQUEST, blocking = true)
    @Transactional
    public void onTwilioTranscriptionCompleted(TwilioGatherTranscriptionCompleted event) {
        System.out.println("Processing gather Request transcription completed: " + event.getCallSid());
    ...
    @ConsumeEvent(value = TwilioGatherTranscriptionCompleted.EVENTNAME_ZIP, blocking = true)
    @Transactional
    public void onTwilioGatherTranscriptionCompleted(TwilioGatherTranscriptionCompleted event) {
        System.out.println("Processing gather Zip transcription completed: " + event.getCallSid());

        TwilioCall call = repository.getOrCreate(event.getCallSid());
        call.zip = event.getTranscriptionText();
